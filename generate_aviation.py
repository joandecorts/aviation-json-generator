...
PUBLISH_KEY = os.environ.get("AVIATION_PUBLISH_KEY", "")
...
def get_json(url, params, retries=3, timeout=30):
    """Crida NOAA/AviationWeather i retorna JSON.

    Protecció afegida:
    - endpoint sense www
    - detecció de resposta buida
    - detecció de resposta no JSON
    - diagnòstic clar si NOAA retorna HTML/error temporal
    """
    last_error = None
        ...
        try:
            r = requests.get(
                ...
                headers={
                    ...
                    "User-Agent": "joandecorts-aviation-json-generator/1.0"
                },
                timeout=timeout
            )
...
", " ")
            ...
            if not (r.text or "").strip():
                raise ValueError(
                    f"Resposta buida NOAA. "
                    f"Status={status} Content-Type={ctype} URL={final_url}"
                )

            if "json" not in ctype.lower():
                raise ValueError(
                    f"NOAA no retorna JSON. "
                    f"Status={status} Content-Type={ctype} URL={final_url} "
                    f"Body={body_preview}"
                )
...
def publish_payload(payload):
    ...
    if not PUBLISH_KEY:
        raise RuntimeError("Falta AVIATION_PUBLISH_KEY als secrets/variables del workflow.")
    r = requests.post(
